# Dataset: Stock Market Manipulation Scheme

**Columns:** trade_id, date, stock_symbol, volume, trade_price_rs, buyer_id, seller_id, trade_type, circuit_filter_hit, price_change_pct, related_group, off_market_trades, pattern_type, insider_trading_flag, sebi_investigation

**Rows:** 15

**Seed:** 42 (deterministic)

## data_sources Format

**Primary Dataset:**
- Synthetic Stock Manipulation Dataset — [GitHub](https://github.com/arora200/aplly_case_db/datasets/067-fraud-stock-manipulation/)
- **Columns:** trade_id, date, stock_symbol, volume, trade_price_rs, buyer_id, seller_id, trade_type, circuit_filter_hit, price_change_pct, related_group, off_market_trades, pattern_type, insider_trading_flag, sebi_investigation
- **Rows:** 15 trades including pump & dump and circular trading patterns

**Note:** Synthetic dataset. Reference: SEBI investigation reports.
